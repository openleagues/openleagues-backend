...
def test_create_user_with_no_email(user_factory):
    ...
    assert str(err.value) == "User email required"
...
def test_super_user_is_not_superuser(db, user_factory):
...
def test_create_user_with_no_firstname(user_factory):
    ...
    assert str(err.value) == "Users must have first name"
...
def test_create_user_with_no_lastname(user_factory):
    ...
    assert str(err.value) == "Users must have last name"
...
def test_create_superuser_with_no_email(user_factory):
    ...
    assert str(err.value) == "Admin email required"
...
def test_user_email_incorrect(user_factory):
    ...
    with pytest.raises(ValueError) as err:
        user_factory.create(email="exampletest.com")
